...
>(
  ...
  async (formData: FormData, { rejectWithValue }) => {
    try {
      ...
      console.log("seller image updated successfully", response.data);
      return response.data;
    } catch (error: any) {
      if (axios.isAxiosError(error) && error.response) {
        console.error(
          "Update seller image error response data:",
          error.response
        );
        return rejectWithValue(error.message);
      } else {
        console.error("Update seller image error message:", error);
        return rejectWithValue("Failed to update seller image");
      }
    }
  }
);
...
const sellerSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // update seller image
      .addCase(updateSellerImage.pending, (state) => {
        state.loading = true;
        state.error = null;
        state.profileUpdated = false;
      })
      .addCase(
        updateSellerImage.fulfilled,
        (state, action: PayloadAction<Seller>) => {
          if (state.profile) {
      ...
      .addCase(updateSellerImage.rejected, (state, action) => {
        state.loading = false;
        state.error = (action.payload as string) || "Failed to update seller image";
      })
      // fetch sellers
